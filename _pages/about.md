  ...
  function initPubFilter() {
    ...
    for (var i = 0; i < filterBtns.length; i++) {
      filterBtns[i].onclick = function() {
        ...
        // Filter list papers - find all paragraphs and line breaks between h3 headers
        var allElements = document.querySelectorAll('h3, p, br');
        ...
        for (var m = 0; m < allElements.length; m++) {
          ...
          if (elem.tagName === 'H3' && elem.textContent.indexOf('Conference') !== -1) {
            inPublications = true;
            continue;
          }
          
          // Stop at Resources section
          if (elem.tagName === 'H3' && elem.textContent.indexOf('Resources') !== -1) {
            inPublications = false;
            continue;
          }
          
          // Skip non-paragraph elements
          if (!inPublications || elem.tagName !== 'P') continue;
          
          // Check if this paragraph has a first-author comment before it
          var prevSibling = elem.previousElementSibling;
          var isFirstAuthor = false;
          
          // Check for comment node between this and previous element
          var node = elem.previousSibling;
          while (node && node !== prevSibling) {
            if (node.nodeType === 8 && node.data.indexOf('first-author') !== -1) {
              isFirstAuthor = true;
              break;
            }
          ...
          // Also check inline HTML for comment
          if (!isFirstAuthor) {
            var html = elem.outerHTML;
            if (html.indexOf('<!-- first-author -->') !== -1) {
              isFirstAuthor = true;
            }
          }
