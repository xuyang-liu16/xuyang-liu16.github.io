...
- 🎬 **Video Understanding**: long/audio-video, and streaming reasoning via efficient encoding and compression, including [VidCom<sup>2</sup>](https://arxiv.org/abs/2505.14454)<sub>[EMNLP'25]</sub>, [STC](https://arxiv.org/abs/2512.00891)<sub>[CVPR'26]</sub>, and [OmniSIFT](https://arxiv.org/abs/2602.04804).
...
<div style="margin:15px 0;">
  <button id="btn-all" onclick="filterPub('all')" style="padding:4px 12px;border:1px solid #00369f;background:#00369f;color:#fff;border-radius:4px;cursor:pointer;margin-right:8px;font-size:13px;">All</button>
  <button id="btn-first" onclick="filterPub('first')" style="padding:4px 12px;border:1px solid #ccc;background:transparent;border-radius:4px;cursor:pointer;font-size:13px;">First Author</button>
</div>

<style>
.pub-btn:hover { background:#00369f !important; color:#fff !important; border-color:#00369f !important; }
</style>
...
function filterPub(type) {
  ...
  if (type === 'all') {
    ...
    btnFirst.style.cssText = 'padding:4px 12px;border:1px solid #ccc;background:transparent;border-radius:4px;cursor:pointer;font-size:13px;';
  } else {
    btnAll.style.cssText = 'padding:4px 12px;border:1px solid #ccc;background:transparent;border-radius:4px;cursor:pointer;margin-right:8px;font-size:13px;';
  ...
  // 获取所有论文条目
  var papers = document.querySelectorAll('.pub-entry');
  papers.forEach(function(paper) {
    if (type === 'all') {
      paper.style.display = '';
    } else {
      // First Author 只显示 data-first="true" 的论文
      if (paper.getAttribute('data-first') === 'true') {
        paper.style.display = '';
      } else {
